...
def collate_fn_mmg(batch):
    ...
    rel_label_list = []
    edge_indices, descriptor,gt_obj_scene, gt_rel_scene = [], [], [], []
    batch_ids = []
    ...
    for i, b in enumerate(batch):
        ...
        gt_obj_scene.append(b[6])
        gt_rel_scene.append(b[7])
    ...
    return torch.cat(obj_point_list, dim=0), torch.cat(rel_point_list, dim=0), torch.cat(obj_label_list, dim=0), \
         torch.cat(rel_label_list, dim=0), torch.cat(edge_indices, dim=0), torch.cat(descriptor, dim=0), \
          torch.cat(gt_obj_scene, dim=0), torch.cat(gt_rel_scene, dim=0), torch.cat(batch_ids, dim=0)

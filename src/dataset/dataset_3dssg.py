...
class SSGDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        # add #
        self.scene_type = ["living room", "bed room", "dining room", "toilet",
                           "kitchen room", "storage room", "desk room", "room"]
        self.scene_to_idx = {scene: idx for idx, scene in enumerate(self.scene_type)}  # {living room : 0,1,2,3,....,
        room_path = os.path.join(self.mconfig.root, "room_type.json")
        with open(room_path, 'r') as file:
            self.scene_infos = json.load(file)
    ...
    def __getitem__(self, index):
        ...
        scene_label = self.scene_infos[scan_id_no_split]
        map_scene2Idx = self.scene_to_idx[scene_label]
        ############################ ********* ############################
        ...
        obj_num = gt_class.size(0)
        rel_num = gt_rels.size(0)
        gt_obj_scene = torch.tensor([map_scene2Idx for i in range(obj_num)], dtype=torch.long)
        gt_rel_scene = torch.tensor([map_scene2Idx for i in range(rel_num)], dtype=torch.long)
        ...
        while(len(rel_points) == 0 or gt_rels.sum()==0) and self.for_train:
            ...
            if self.use_descriptor:
                if self.use_2d_feats:
                    obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, gt_obj_scene, gt_rel_scene = self.__getitem__(index)
                else:
                    obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, gt_obj_scene, gt_rel_scene = self.__getitem__(index)
            else:
                obj_points, rel_points, gt_class, gt_rels, edge_indices, gt_obj_scene, gt_rel_scene = self.__getitem__(index)

        if self.use_descriptor:
            if self.use_2d_feats:
                return obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor
            return obj_points, rel_points, gt_class, gt_rels, edge_indices, descriptor, gt_obj_scene, gt_rel_scene

        return obj_points, rel_points, gt_class, gt_rels, edge_indices, gt_obj_scene, gt_rel_scene
    ...
    def data_preparation(self, points, instances, num_points, num_points_union, scene_id="",
                     ...
                     padding=0.2, num_max_rel=-1, shuffle_objs=True, all_edge=False):
        #, use_2d_feats=False, multi_view_root=None):
        #all_edge = for_train
        # get instance list

...
class BaseModel(nn.Module):
    ...
    def loadConfig(self, path):
        if os.path.exists(path):
            ...
            return data['iteration'], eva_res, data['epoch']
        else:
            return 0, 0
    ...
    def save(self):
        ...
        if not os.path.exists(self.config_path+self.best_suffix):
            ...
            _, eva_res, _ = self.loadConfig(self.config_path+self.best_suffix)
    ...
    def load(self, best=False):
        ...
        if best:
            ...
            if os.path.exists(self.config_path+self.best_suffix) and best:
                ...
                iteration, _, _= self.loadConfig(self.config_path + self.suffix)
                iteration_best, _,_= self.loadConfig(self.config_path + self.best_suffix)
        ...
        self.iteration, self.eva_res, self.epoch = self.loadConfig(self.config_path + suffix)
        for name,model in self._modules.items():
            skip = False
            for k in self.skip_names:
                if name.find(k) != -1:

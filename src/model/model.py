...
from src.model.SGFN_L3DSG.model import L3DSGModel
...
class L3DSGNet():
    def __init__(self, config):
        ...
        ''' Build Model '''
        self.model = L3DSGModel(self.config, num_obj_class, num_rel_class,self.dataset_train.classNames,
                            self.dataset_train.relationNames).to(config.DEVICE)
    ...
    def data_processing_train(self, items):
        obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor, gt_obj_scene, gt_rel_scene, batch_ids = items
        obj_points = obj_points.permute(0,2,1).contiguous()
        rel_points = rel_points.permute(0, 2, 1).contiguous()
        obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor, gt_obj_scene, gt_rel_scene, batch_ids = \
            self.cuda(obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor, gt_obj_scene, gt_rel_scene,batch_ids)
        return obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor,gt_obj_scene, gt_rel_scene, batch_ids
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in loader:
                ...
                ''' get data '''
                obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor,\
                gt_obj_scene,gt_rel_scene, batch_ids = self.data_processing_train(items)



                logs = self.model.process_train(gt_obj_scene,gt_rel_scene, obj_points, rel_points, gt_class, descriptor, gt_rel_cls, edge_indices, batch_ids, with_log=True,
    ...
    def validation(self, debug_mode = False):
        ...
        for i, items in enumerate(val_loader, 0):
            ''' get data '''
            obj_points, rel_points, gt_class, gt_rel_cls, edge_indices, descriptor, gt_obj_scene, gt_rel_scene, batch_ids = self.data_processing_val(items)
            # logs = self.model.process_train(gt_obj_scene,gt_rel_scene, obj_points, rel_points, gt_class, descriptor, gt_rel_cls, edge_indices, batch_ids, with_log=True,
            #                                                 weights_obj=self.dataset_train.w_cls_obj,
            #                                                 weights_rel=self.dataset_train.w_cls_rel,
            #                                                 ignore_none_rel = False)
            ...
            with torch.no_grad():
                ...
                # else:
                top_k_obj, top_k_rel, tok_k_triplet, cls_matrix, sub_scores, obj_scores, rel_scores \
                    = self.model.process_val(gt_obj_scene,gt_rel_scene, obj_points,rel_points,
                                             gt_class, descriptor, gt_rel_cls, edge_indices, batch_ids, use_triplet=True)

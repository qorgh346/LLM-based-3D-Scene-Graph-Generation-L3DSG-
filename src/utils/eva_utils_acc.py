...
def evaluate_triplet_topk(objs_pred, rels_pred, gt_rel, edges, multi_rel_outputs, topk, confidence_threshold=0.5, epsilon=0.02, use_softmax=False, obj_topk=None):
    res, triplet = [], []
    if not use_softmax:
        # convert the score from log_softmax to softmax
        objs_pred = np.exp(objs_pred)
        ...
        objs_pred = objs_pred * 1 #F.softmax(objs_pred, dim=-1)
